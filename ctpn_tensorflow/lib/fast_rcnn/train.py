...
def get_training_roidb(imdb):
    ...
    if cfg.TRAIN.USE_FLIPPED:                                              # 使用水平翻转来扩充数据
    ...
    if cfg.TRAIN.HAS_RPN:
            rdl_roidb.prepare_roidb(imdb)   # 用RPN来检测目标,这里是true
    else:
...
def train_net(network, imdb, roidb, output_dir, log_dir, pretrained_model=None, max_iters=40000, restore=False):
    ...
    with tf.Session(config=config) as sess:
        ...
        print('Solving...')
        writer = tf.summary.FileWriter('./tensor_log')
